...
struct SlabData {
    ...
    union {
        unsigned long long ilock;
        char p[128];
    }; // 128 bytes
    unsigned long long keyValue[32]; // 256 byte
 ...
 * @param slabs
 */
__forceinline__ __device__ void LockSlab(const unsigned long long& next, const unsigned &src_bucket, const unsigned& laneId, volatile SlabData **slabs){

    if(laneId == 0){
        auto ilock = (unsigned long long*)(next == BASE_SLAB ? &(slabs[src_bucket]->ilock) : &(((SlabData*)next)->ilock));
 ...
 * @param slabs
 */
__forceinline__ __device__ void UnlockSlab(const unsigned long long& next, const unsigned &src_bucket, const unsigned& laneId, volatile SlabData **slabs){
...
__forceinline__ __device__ volatile unsigned long long *
        ...
        if (masked_ballot != 0) {
            if (src_lane == laneId) {
                ...
                volatile unsigned long long *addr =
                        SlabAddress(next, src_bucket, dest_lane, slabs, num_of_buckets);
                unsigned long long old_pair = *addr;
                if(old_pair == 0){
                    *addr = newPair;
                }
                if (old_pair == 0) {
            ...
            if (next_ptr == 0) {
                ...
                if (laneId == ADDRESS_LANE - 1) {
                    auto* slabAddr = (unsigned long long*)SlabAddress(next, src_bucket, ADDRESS_LANE - 1, slabs, num_of_buckets);
                    unsigned long long temp = *slabAddr;
