...
void populateUnsignedUnsigned(NoCacheKVStoreClient<unsigned, unsigned, kvgpu::SimplModel<unsigned>> &client, unsigned *seed, int n, int range);
...
int main() {
    NoCacheKVStoreCtx<unsigned, unsigned, kvgpu::SimplModel<unsigned>> ctx;
    ...
    NoCacheKVStoreClient<unsigned, unsigned, kvgpu::SimplModel<unsigned>> client(ctx);
    ...
    std::cout << "Throughput (Mops) " << BATCHSIZE * times.size() / seconds / 1e6 << std::endl;
